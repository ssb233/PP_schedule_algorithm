...
class Task:
    def __init__(self,task_type, model_id=1, micro_batch_id=1, pp_stage_id=1):
        ...
        self.start_time = 0
        self.end_time = 0
        self.pre_task = [] # 前驱任务,依赖关系,只有前驱任务都满足了才能调度
...
class Schedule:
    ...
    def add_constraint(self, constraint, model_id):
        ...
        A_tuple = (model_id, A_pp_stage_id, A_micro_batch_id)
        B_task = None
        for task in self.task_list:
            if task.model_id == model_id and task.pp_stage_id == B_pp_stage_id and task.micro_batch_id == B_micro_batch_id:
                B_task = task
        if  B_task is None:
            raise ValueError("Task not found bro!")
        B_task.pre_task.append(A_tuple) # 添加前驱约束
    ...
    def add_model_tasks(self, model_id, micro_batch_num, pp_start_layer, pp_total_num, is_reversed=False):
        ...
        for i in range(dp_groups):
            for j in range(pp_start_layer + i * pp_total_num * step, pp_start_layer + (i + 1) * pp_total_num * step, step):
                for k in range(1, micro_batch_num + 1):
                    task_forward = Task('forward', model_id, k, j)
                    task_backend = Task('backward', model_id, k + micro_batch_num, j)
    ...
    def greedy_init_matrix(self):
        ...
        finished_tuple_list = [] # 记录已经完成的任务的元组列表，一个任务完成就加入(model_id, pp_stage_id, micro_batch_id)
        while(len(self.remained_task)> 0):
            ...
            # 每一轮我们维护一个可调度任务列表，然后从中选取一个可调度任务进行调度
            feasible_task_list = []
            for task in self.remained_task:
                result = task.update_feasibility(finished_tuple_list)
                if result == True:
            ...
            if target_task is None:
                ...
                self.scheduled_task.append(target_task)
                self.remained_task.remove(target_task)
                finished_tuple_list.append((target_task.model_id, target_task.pp_stage_id, target_task.micro_batch_id))
